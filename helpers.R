...
getURL <- function(base_url, api_key, 
                   category="Everything", borough="Everywhere", 
                   ll="40.7127,-74.0059", radius=1, limit=20) {
    
    filters <- ""
    loc <- ""
    rad <- ""
    
    if(borough=="Everywhere" ) {
        loc <- paste0("ll=", ll)
        rad <- paste0("radius=", radius*1600)
    } else {
        filters <- paste0("filters=", paste0("borough:", borough))
    }
    if(category != "Everything") {
        if(filters=="") {
            filters <- paste0("filters=", paste0("category:", category))
        } else {
            filters <- paste0(filters, paste0(",category:", category))
        }    
    }
    ...
    url <- paste(base_url, filters, loc, rad, limit, api_key, sep="&")
    return(gsub("&&+", "&", url))
}
...
retrieveEvent <- function(url) {
    ...
    picks <- as.numeric(event_raw$geocode_latitude) > 40.3127 & 
        as.numeric(event_raw$geocode_latitude) < 41.1127 &
        as.numeric(event_raw$geocode_longitude) < -73.3059 &
        as.numeric(event_raw$geocode_longitude) > -74.4059
